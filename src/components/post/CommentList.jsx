          ...
          <div className="comment-header">
            ...
            <span className="comment-date">{comment.regDate}</span>
                ...
                <div className="flex justify-end items-center">
                  {user.nickname === comment.nickname && editMode !== comment.id && (
                    <button
                  ...
                  {(user.nickname === comment.nickname || user.role === "ADMIN") && (
                    <CommentDelete
                      id={comment.id}
                      hasReplies={comment.subComments.length > 0}
                    ...
                    <ul>
                      {comment.subComments.map((subComment) => (
                        <li key={subComment.id} style={{ marginLeft: `${subComment.level * 20}px` }}>
                          ...
                          {/* 대댓글 수정/삭제 버튼 */}
                          <div className="flex justify-end items-center">
                            {user.nickname === subComment.nickname && editMode !== subComment.id && (
                              <button
                                onClick={() => handleEditToggle(subComment.id)}
                                className="comment-action-btn"
                              >
                                수정
                              </button>
                            )}
                            {(user.nickname === subComment.nickname || user.role === "ADMIN") && (
                              <CommentDelete
                                id={subComment.id}
                                onDeleteSuccess={handleDeleteSuccess}
                              />
